 ...
 */
//void main(void) __attribute__((noreturn));  This doesn't remove int warning.
int main( void )
{
...
// Output bit pattern on LED pin PORTA7
		for(i=0; i<8; i++)
		{
			if(temp & 0x80)
			{
...
//        USI_TWI_Transmit_Byte(temp);		// Echo back data. Wait for READ?..yes..blah
    }
